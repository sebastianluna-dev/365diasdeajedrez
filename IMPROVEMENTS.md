...
*Mostly resolved:* every write action resolves the user on the server through the DAL (which now
...
*Partially resolved (2026-09-09):* `Academia de Ajedrez Landing.zip` removed from the tree and
...
### 39. Nothing verifies the code before it reaches `main`: no CI, no `typecheck`, prettier undeclared — [DX]
*Mostly resolved (2026-09-09):* `.github/workflows/ci.yml` runs `npm ci`, `db:generate`, `lint`,
...
**What is left:** `format:check` is not in CI because today it would fail: Prettier would change 135
files. Formatting the whole repo is a commit of its own, with no logic inside (see `todos.md`); once
that is done, add the step to the workflow.
...
Prisma 7 with the PostgreSQL *driver adapter* does **not** fill `meta.target` on uniqueness
...
*Mostly resolved (2026-09-09):* `studyDetailInclude.games` uses `select` with the ten fields the
...
*Mostly resolved (2026-09-09):* `lib/logger.ts` (one JSON line per event on `stderr`, without
...
*Mostly resolved (2026-09-09):* `sharp` installed and passed to `buildConfig`, so new uploads fill
...
*Mostly resolved (2026-09-09):* `include` with `{ts,tsx}`, `@vitest/coverage-v8` and
...
*Partially resolved (2026-09-09):* `target: ES2022` and `noUnusedLocals: true`, both without a
single error.
...
*Mostly resolved (2026-09-09):* a "Copias de seguridad" section in the README with the `pg_dump` of
...
*Partially resolved:* down from 21 to **10** (1 low, 6 moderate, 3 high) by removing Tailwind and
...
*Investigated:* the most likely cause of the flag was Tailwind v4, which **has already been
...
*Mostly resolved with entry 25:* the reference instant is already stable within the day and can be
...
*Tried and discarded (2026-09-09):* `experimental.cssChunking: "graph"` left the home page with the
...
| 9 | Half-removed Tailwind | Removed (dependencies, `@import`, `@theme`, `postcss.config.mjs` and the utility classes of the layouts). Its *preflight* was replaced by an equivalent explicit reset in `globals.css` so as not to change anything visually. |
