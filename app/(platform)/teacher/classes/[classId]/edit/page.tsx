import type { Metadata } from "next";
import { notFound } from "next/navigation";
import { ClassFormSection } from "@/components/sections/platform/teacher/classes/class-form.section";
import { requireTeacher } from "@/lib/platform-auth/roles";
import { getPlatformDb } from "@/lib/platform-db/get-platform-db";
import { safeTimeZone } from "@/lib/timezone";
import { getTeacherClassDetail, listMeetingProviders } from "@/services/teacher-classes/teacher-classes.service";
import { getAssignedStudents } from "@/services/teacher-students/teacher-students.service";

interface EditClassPageProps {
  params: Promise<{ classId: string }>;
  searchParams: Promise<{ error?: string }>;
}

export async function generateMetadata({ params }: EditClassPageProps): Promise<Metadata> {
  const { classId } = await params;
  const classDetail = await getTeacherClassDetail(classId);
  return classDetail ? { title: `Editar ${classDetail.title}` } : {};
}

export default async function EditClassPage({ params, searchParams }: EditClassPageProps) {
  const { teacher } = await requireTeacher();
  const { classId } = await params;

  const classDetail = await getTeacherClassDetail(classId);
  if (!classDetail) notFound();

  const [students, providers, teacherRow, { error }] = await Promise.all([
    getAssignedStudents(),
    listMeetingProviders(),
    getPlatformDb().teacher.findUnique({ where: { id: teacher.id }, select: { timezone: true } }),
    searchParams,
  ]);

  return (
    <div className="platform-page teacher-edit-class-page">
      <header className="platform-page__head">
        <h1 className="platform-page__title">Editar clase</h1>
        <p className="platform-page__subtitle">{classDetail.title}</p>
      </header>

      <ClassFormSection
        classDetail={classDetail}
        students={students}
        providers={providers}
        timeZone={safeTimeZone(teacherRow?.timezone)}
        errorCode={error}
      />
    </div>
  );
}
