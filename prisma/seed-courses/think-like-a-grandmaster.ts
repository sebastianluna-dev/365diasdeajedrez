// Course "Piense como un gran maestro": the 162 diagrams of Alexander Kotov's
// book, spread over its 5 chapters, as position lessons.
//
// THE CONTENT IS NOT WRITTEN HERE. The canonical source is the import package
// `data/think-like-a-grandmaster.import.json`; this module only validates it and
// translates it into the `SeedCourse` shape prisma/seed.ts consumes. To change
// the content the package is regenerated, this file is not edited: that is why
// the lessons carry no fixed id written by hand, but one derived from their
// `stableKey` (see `deterministicId`).
//
// ON THE SOURCES. The positions and the moves are facts and are cited freely.
// The commentary the package attaches to each diagram, on the other hand, is a
// literal transcription of the book — Ediciones Polo's translation, 2016, still
// in copyright — and travels inside the lesson's PGN. It is third-party
// material: it is fine for the academy's internal use, not for publishing it
// without a licence.
...
/** Fixed ids, in ranges of their own so as not to clash with seed-data.ts's. */
export const GM_IDS = {
  course: "10000003",
...
/**
 * Checks the package before it reaches the database: declared counts,
 * uniqueness of the keys and legality of each position. It runs on every seed
 * on purpose — 162 FENs parse in milliseconds — so a regenerated package cannot
 * slip in an impossible position without anything going off.
 */
...
/**
 * Two diagrams arrive from the package with the turn inverted: the side that
 * does not move is left in check, so the position is illegal as it stands. In
 * both cases the book's own text says whose move it is — diagram 6 comes from
 * `16.Rxe6+` and 60 from `1…Kxa5` — so this does not correct the board by
 * guesswork: it only restores the turn the automatic inference got wrong. Any
 * other FEN field is left intact.
 */
...
/** FEN of the lesson with the turn corrected if it is in the list above. */
...
/**
 * The PGN is the single source of the lesson's content (moves, commentary and
 * the `SourcePDFPage`, `DiagramNumber`, `White`/`Black`… headers), so it is kept
 * verbatim. The only thing touched is the FEN header of the corrected diagrams:
 * that header is NOW the only source of the lesson's position, so a wrong FEN
 * there is a wrong diagram.
 */
...
// ---------------------------------------------------------------------------
// Translation to SeedCourse
// ---------------------------------------------------------------------------

/**
 * Stable id derived from the lesson's `stableKey` (UUID v5 over SHA-1). The seed
 * upserts by id, so reimporting the same package always lands on the same row
 * and never duplicates; and since it does not depend on the order, moving a
 * lesson elsewhere does not turn it into a different one.
 */
const ID_DIGITS = "0123456789";

/**
 * Eight characters, because the lesson's id goes in the URL. It still derives
 * from the `stableKey`, so reimporting the package lands on the same rows.
 *
 * Eight digits are 100 million values for 162 lessons. The probability of two
 * keys giving the same number is not entirely negligible — the birthday problem
 * — so the importer checks it at startup instead of letting two lessons
...
/** Minutes per diagram; the chapter's is the sum of its own. */
const MINUTES_PER_LESSON = 6;

/**
 * Short record of the lesson from the package's metadata. It does not summarise
 * the book's commentary on purpose: it is third-party text and already travels
 * in the PGN.
 */
...
function toSeedLesson(lesson: ImportLesson, topics: string[]): SeedLesson {
  ...
  return {
    ...
    // Each lesson is a diagram with its commentary, not a game to be walked
    // through: the PGN carries no moves, only the starting position.
    orientation: fen.split(" ")[1] === "b" ? BOARD_ORIENTATION.BLACK : BOARD_ORIENTATION.WHITE,
    pgn: resolvePgn(lesson),
    topics,
    // A lone diagram has no forced line to train: the exercises will come when the
    // variations of each position are annotated.
    exercises: [],
  };
}
...
// ---------------------------------------------------------------------------
// Master games of the course
// ---------------------------------------------------------------------------
//
// These games are NOT lessons: they make up the course's game database, which
// is also the corpus of the position search. They are grouped by the chapter
// whose topic they illustrate.
// ---------------------------------------------------------------------------
// Chapter 1 — Analysis of variations
// ---------------------------------------------------------------------------

/**
 * Boleslavsky–Flohr, 1950. The sacrifice on e6 and, above all, the range of
 * replies to 18.Qh5+: the game serves to teach ENUMERATING the opponent's moves
 * before calculating any of them.
 */
...
/**
 * Alekhine–Réti, Vienna 1922. Five short replies to 12...c5: the example of a
 * wide and shallow tree, where the hard part is not calculating deep but not
 * leaving out a single candidate.
 */
...
/**
 * Rauzer–Riumin, Leningrad 1936. A long plan of file and diagonal: it serves
 * the lesson on candidate moves and reappears in chapter 2.
 */
...
// ---------------------------------------------------------------------------
// Chapter 2 — Positional judgement
// ---------------------------------------------------------------------------

/** Plater–Botvinnik, 1947. One open file is enough to win. */
...
/** Stahlberg–Taimanov, Zurich 1953. The rook on the seventh that paralyses. */
...
/** Makagonov–Botvinnik, Sverdlovsk 1943. Squares of one colour, without their bishop. */
...
/** Kotov–Taimanov, Zurich 1953. The knight that went to the edge. */
...
/** Botvinnik–Alekhine, AVRO 1938. One queen move and the knight does not come back. */
...
/** Botvinnik–Yudovich, 1933. Five pieces tied to a single badly placed one. */
...
/**
 * Romanovsky–Vilner, 1927. A target chosen early (the f5 square) and every move
 * subordinated to it. The main line stops at move 35, where the plan is already
 * accomplished.
 */
...
/** Sokolsky–Botvinnik, 1938. Developing is not a plan. */
...
/** Petrosian–Euwe, Zurich 1953. Moves without a common goal and their bill. */
...
/**
 * Taimanov–Bronstein, Zurich 1953. A pawn sacrificed in the opening for two
...
/**
 * The course's master games, as a database of their own. Besides being
 * consultable from "Mis estudios", it is the corpus that feeds the position
 * search: without real games, the explorer has nothing to find.
 */
export const THINK_LIKE_A_GRANDMASTER_DATABASE = {
  ...
  // To the first chapter: collections are per chapter, and the fine distribution
  // among the five is done by `scripts/migrate-chapter-collections.ts`.
